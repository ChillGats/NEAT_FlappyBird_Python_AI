...
1. `bird_y`: Normalized vertical position of the agent.
2. `bird_vel_y`: Vertical velocity of the agent.
3. `next_pipe_dist_x`: Horizontal distance to the upcoming obstacle.
4. `next_pipe_top_y`: Upper bound of the upcoming pipe gap.
5. `next_pipe_bottom_y`: Lower bound of the upcoming pipe gap.
...
- **Decision Rule**: Action = Jump ($1$) if $y > 0.5$, else Coast ($0$).
